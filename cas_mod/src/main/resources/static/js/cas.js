...
function writeToLocalStorage(browserStorage) {
  if (typeof Storage === "undefined") {
    ...
    console.log(
      `Stored ${browserStorage.payload} in local storage under key ${browserStorage.context}`
    );
  }
}
...
function writeToSessionStorage(browserStorage) {
  if (typeof Storage === "undefined") {
    ...
    console.log(
      `Stored ${browserStorage.payload} in session storage under key ${browserStorage.context}`
    );
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function checkFields() {
    const isUsernameEmpty = !usernameInput.value.trim();
    const isPasswordEmpty = !passwordInput.value.trim();

    // Disable button if either field is empty
    submitButton.disabled = isUsernameEmpty || isPasswordEmpty;
  }
